...
class DefaultClient:
    ...
    def build_headers(self, request=None):
        ...
        headers[constant.X_CA_TIMESTAMP] = utils.get_timestamp()
        ...
        if constant.POST == request.get_method() and constant.CONTENT_TYPE_JSON == request.get_content_type():
            headers[constant.HTTP_HEADER_CONTENT_MD5] = utils.get_md5_base64_str(request.get_body())
            str_to_sign = utils.build_sign_str(
                uri=request.get_url(),
                method=request.get_method(),
                headers=headers
            )
        ...
        headers[constant.X_CA_SIGNATURE] = utils.sign(str_to_sign, self.__app_secret)
